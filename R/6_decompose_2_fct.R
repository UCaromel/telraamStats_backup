...
decompose_2_fct <- function(data, sensor1,sensor2, hour_x, direction1, direction2, mobility, norm, vacations=NULL, public_holidays=NULL){
  ...
  result <- data.frame(d1$trend,d2$trend,d1$seasonal,d2$seasonal,d1$random,d2$random)

  if (norm=="Oui"){ # normalisation
    result <- data.frame(apply(result,2,scale))
  }

  lab_sensor1 <- paste(sensor1,direction1)
  lab_sensor2 <- paste(sensor2,direction2)

  trend_random <- data.frame(trend = c(result$d1.trend,result$d2.trend),
